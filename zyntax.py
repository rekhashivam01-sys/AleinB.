...
from core.Zyntax import Zyntax as Yuna 
...
def print_startup_banner ():
    """Print the Zyntax startup banner"""
    banner = [
        "Zyntax Multipurpose Bot",
        "──────────────────────────────────────",
        "System: Operational",
        "Developer: void & codez dev (Codez Dev)",
...
def print_system_ready ():
    ...
    logger.success("SYSTEM", "Zyntax is now online and ready to serve!")
        ...
        try :
            ...
            await self .db .execute ("""
                CREATE TABLE IF NOT EXISTS tickets (
                    ...
                    embed_footer TEXT DEFAULT 'Powered by Codez Dev',
    ...
    if context .guild is not None :
        try :
            async with aiohttp .ClientSession ()as session :
                ...
                command_details =f"› **Command Name:** {executed_command}\n› **Guild Name:** {context.guild.name} ({context.guild.id})\n› **Channel Name:** {context.channel.name} ({context.channel.id})\n› **User:** [{context.author}](https://discord.com/users/{context.author.id}) ({context.author.id})"
